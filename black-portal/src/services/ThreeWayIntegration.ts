...
const INTEGRATION_CONFIG = {
  blackPortal: {
    baseUrl: process.env.NEXT_PUBLIC_BLACK_PORTAL_URL || 'https://black.trademate.ai',
    apiKey: process.env.BLACK_PORTAL_API_KEY,
  },
  tradematePlatform: {
    baseUrl: process.env.TRADEMATE_PLATFORM_URL || 'https://app.trademate.ai',
    apiKey: process.env.TRADEMATE_API_KEY,
  },
  partnerPortal: {
    baseUrl: process.env.PARTNER_PORTAL_URL || 'https://partner.trademate.ai',
...
export class ThreeWayIntegrationService extends EventEmitter {
  ...
  constructor() {
    ...
    this.platformClient = this.createApiClient('tradematePlatform');
  ...
  private mapServiceName(service: string): 'black_portal' | 'trading_platform' | 'support_portal' {
    const mapping = {
      ...
      tradematePlatform: 'trading_platform' as const,
